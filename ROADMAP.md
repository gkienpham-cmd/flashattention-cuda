...
- `[~]` **v8.7 — score-stationary inner loop** (CUDA-core, T4) ★ *remove, don't hide* — code complete, T4 gate pending. Flip the inner-loop layout to **lane = key**: each lane computes the full dot product q·k_c (no per-key cross-lane reduction), softmax runs **once per 32-key group** (recurrence 32× shorter), PV is a transpose via pipelined `__shfl` broadcasts. **FP16 transposed smem** (~18 KB → ~3 blocks/SM) holds occupancy so the **layout is the only variable** (clean A/B vs `v8_gqa_occ`). **Roofline BLIND** (AI=2G/b). **Prediction: µs/tok drops, best at d=64; d=128 at risk of smem-BW-bound; if µs/tok drops but %HBM stays ~10% → floor is load latency → v9 FP8 next.** Run `notebooks/v8_7_score_stationary_gate.ipynb`. See `results.md`/`decisions.md` Step 8.7, `interview-prep.md` C11.6.
