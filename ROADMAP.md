...
- `[~]` **v8.6 — hide the reduction latency** (CUDA-core, T4) — code complete, T4 gate pending. v8.5 isolated the floor to the **per-key warp-shuffle reduction + serial online-softmax recurrence** (not the load). v8.6 is a **2-arm single-variable ablation** to *hide* it: **Arm 1 `v8_gqa_occ`** (FP16 smem → 4 blocks/SM, 2× resident warps) and **Arm 2 `v8_gqa_ilp`** (KU=4-unrolled key loop → pipeline independent shfl reductions). **Roofline is BLIND** (AI=2G/b, floor, limiter identical to Cut 1 — a *schedule* claim the model can't see). **Prediction: occupancy > ILP** (TLP hides the whole serial chain; ILP only the reductions); **if both null → the floor is the serial recurrence → score-stationary redesign (v8.7), and v9 stays premature.** Run `notebooks/v8_6_reduction_gate.ipynb`. See `results.md`/`decisions.md` Step 8.6, `interview-prep.md` C11.5.
