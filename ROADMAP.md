...
- `[ ]` **v8.5 — close the per-CTA gap (cp.async double-buffer + occupancy)** — push the CUDA-core M-pack kernel toward bandwidth-bound (overlap KV load w/ compute, raise blocks/SM, vectorize the GEMV reduction). = the "Memory pipeline" item applied to decode. **[BUILD]** (T4-cheap). *The "schedule before bytes" step v8 proved is still needed (≤11% HBM); also the precondition that makes v9 FP8 actually pay.*
- `[ ]` **v9 — FP8 KV cache** — first byte cut (`b:2→1`, `AI ×2`); accuracy delta vs FP16 documented. **Bytes only pay once bandwidth-bound** — v8 is still ≤11% HBM (per-CTA-bound), so **v8.5 (schedule) comes first.** **[B300]** (H100 ok for correctness).
