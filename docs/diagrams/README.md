...
**v9 Task 1 figure (regime characterization — generated by `notebooks/v9_task1_regime.ipynb`):**

| File | What it shows |
|---|---|
| `v9-task1-regime.svg` | **Earns the bandwidth verdict.** Achieved %HBM vs N_k (log-x, B=1, H_kv=1, **L2-flushed, clocks locked**) for `v8_gqa_ss` (FP16) and `v9_fp8` (FP8), d=64/128. Marks the L2-capacity crossing (KV = 4 MB) and the ~70%-of-peak achievable ceiling; `L2!` annotations flag rows where the counter-free test fired (`eff_bw > HBM peak ⇒ L2-served`). Climbs past L2 → memory-bound; stays ~10% with `L2served=False` → per-CTA-bound, confound-free. **This is the only repo figure generated programmatically (matplotlib `savefig`), not hand-authored.** |
